...
$(ENV):
	mamba create -p $(ENV) -y -c conda-forge pygeoprocessing voila jupyter wget

$(INTER):
	mkdir intermediate

$(INTER)/Base_Data.zip: $(INTER) $(ENV)
	$(ENV)/bin/wget -O $(INTER)/Base_Data.zip https://storage.googleapis.com/releases.naturalcapitalproject.org/invest/3.13.0/data/Base_Data.zip
...
$(INTER)/rescaled.tif: $(INTER)/global_dem.tif
	$(ENV)/bin/python rescale.py $(INTER)/global_dem.tif $(INTER)/rescaled.tif
